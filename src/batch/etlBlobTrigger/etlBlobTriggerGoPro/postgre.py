...
def insert_log_etl_df(log_data:pd.Series,cursor:object,connection:object,container:object)->str:
    ...
    cursor.execute("INSERT INTO logs.etl (id, campaign_id,media_id,media_name,initiated_on,finished_on,status,container ) VALUES (DEFAULT,%s,%s,%s,now(),now(),%s,%s) RETURNING id;",(campaign_id,media_id,media_name,status,container))
    connection.commit()
    row_id = cursor.fetchone()[0]
    return row_id
